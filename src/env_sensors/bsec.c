...
LOG_MODULE_REGISTER(bsec, CONFIG_GCLOUD_LOG_LEVEL);
 ...
 */
#define BSEC_STATE_SAVE_INTERVAL	1200
...
/* timer def */
extern struct k_timer iaq_timer;
uint32_t sec = 0;

bool iaq_reading_ok = false;
...
K_WORK_DEFINE(timer_stop_work, timer_stop_work_handler);


void iaq_timer_handler(struct k_timer *dummy)
{
    k_work_submit(&timer_work);
}

void iaq_timer_stop(struct k_timer *dummy)
{
    k_work_submit(&timer_stop_work);
}

/* timer definition */
K_TIMER_DEFINE(iaq_timer, iaq_timer_handler, iaq_timer_stop);
